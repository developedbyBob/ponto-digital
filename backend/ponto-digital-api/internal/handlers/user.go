...
func (h *UserHandler) SetupPin(c *gin.Context) {
    ...
    // Log para debug
    log.Printf("Configurando PIN para usuário %v: %s", userID, req.Pin)
    ...
    if err != nil {
        log.Printf("Erro ao configurar PIN: %v", err)
    ...
    log.Printf("PIN configurado com sucesso para usuário %v", userID)
